...
To implement the wrappers functions in C, I listed out all the LAPACK routines to be used in currently planned Linalg module. Then forward declared wrapper functions for all these routines in `ruby_nmatrix.c`. I also implemented constructors for `nmatrix` struct, namely, `nmatrix_new` and `nmatrix_copy`. These constructors reduced code repetition in wrappers functions as now any new `nmatrix` instance can be declared and initiaized in one line and copy of `nmatrix` instance can be done in one line. 
...
Example invocation for these contructors is `nmatrix* result = nmatrix_new(matrix_qr->dtype, matrix_qr->stype, 2, matrix_qr->count, matrix_qr->shape, NULL);` and `nmatrix* result = nmatrix_copy(matrix);` respectively.
...
Above function `nm_geqrf` is a wrapper for LAPACK routines `sgeqrf`, `dgeqrf`, `cgeqrf` and `zgeqrf`. This function takes the input matrix (or matrices) and relevent arguments, parses them, decalres and initializes result matrices, uses switch statement to decide which LAPACK routine to call and then passes the required arguments to the routine call and finallt returns the resulting matrix (or matrices) as Ruby objects. This wrapper can be called from Ruby code using `NumRuby::Lapack.geqrf(matrix)`. Similarly, other wrapper functions have been implemented.
