...
NumRuby can be viewed as a package for scientific numerical compution in Ruby. It consists of:
...
```ruby
#format: [dimensions legth, time taken by operation]
{
  :addition => [
    [100, 6.0e-06], 
    [2500, 1.0e-05], 
...
`get_index` takes the matrix `nmat` and `indices` (input indices tuple) and #TODO
